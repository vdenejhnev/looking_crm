(function($) {
  ...
  $(document).ready(function() {
    ...
    const chartData = CHART_DATA.map(item => item.count);
    ...
    function drawDealerLeads() {
      $.post(
        '/ajax/front/home.php',
        {
          action: 'get_dealer_leads',
          user_id: $('.user-stat-panel').data('user'),
          year: $('.year-select').val(),
          month: curr_month + 1,
        },
        function(response) {
          console.log(response);
          $('.leads-total-count').text(response.total_count);
          $('.leads-month-count').text(response.total_count_month);
          $('.year-quantity').text(response.total_count_year);
          $('.leads-count-new').text(response.count_new);
          $('.leads-count-single').text(response.count_single);
          $('.leads-count-first').text(response.count_first);
          $('.leads-count-second').text(response.count_second);
          $('.leads-count-done').text(response.count_done);
        },
        'json'
      );
    }
